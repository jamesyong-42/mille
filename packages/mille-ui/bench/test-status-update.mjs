// Phase 5.1 — test-status decoration update-cost harness.
//
// Usage:
//   node bench/test-status-update.mjs
//   PATHS=500 LOOKUP_MS=1 CONCURRENCY=16 node bench/test-status-update.mjs

import { performance } from 'node:perf_hooks';

const { registerTestStatusDecorations, createMapTestStatusClient } =
...
function buildRows(n) {
  ...
  for (let i = 0; i < n; i += 1) {
    rows.push({ id: i + 2, parentId: 1, path: `f${i}.test.ts`, kind: 0 });
  }
  return rows;
}
...
for (let i = 0; i < PATHS; i += 1) {
  const status =
    i % 7 === 0 ? 'failed' : i % 5 === 0 ? 'running' : i % 3 === 0 ? 'skipped' : 'passed';
  initial.push({ path: `f${i}.test.ts`, status });
}
const client = createMapTestStatusClient({ initial });
