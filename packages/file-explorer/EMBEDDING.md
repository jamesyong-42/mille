...
const { createFileExplorerHost } = require('@mille/file-explorer/host');
...
```ts
// renderer.ts
import { connectFileExplorer } from '@mille/file-explorer/client';
...
} from '@mille/file-explorer/react';
```
...
```tsx
// TreeView.tsx
import { useFileExplorerSnapshot } from '@mille/file-explorer/react';
import type { FileExplorer } from '@mille/file-explorer';
...
import type { PortFileExplorer } from '@mille/file-explorer';
...
```ts
import type { DecorationProvider, FileExplorer } from '@mille/file-explorer';
...
```ts
import { FileSystemError, isFileSystemError } from '@mille/file-explorer';
...
`@mille/file-explorer` distributes the native binary via napi-rs's
...
```
npm install --include=optional @mille/file-explorer
```
...
Fix: `npm install --include=optional @mille/file-explorer`, or add the right
...
- **Content search** — filename search ships in this package; content search
  (ripgrep-style) will be a separate `@mille/file-search` package that
  consumes this one.
...
import { FileExplorer, MirrorSnapshot } from '@mille/file-explorer';
import {
  FileSystemError,
  isFileSystemError,
  type ErrorCode,
} from '@mille/file-explorer';
import type {
  Entry,
  EntryId,
  VisibleRow,
...
// UtilityProcess host
import { createFileExplorerHost } from '@mille/file-explorer/host';
import type {
  FileExplorerHost,
  MessagePortLike,
} from '@mille/file-explorer';
...
import {
  ...
  PortFileExplorer,
  PortMirrorSnapshot,
} from '@mille/file-explorer/client';
